...
def zDepthUnfiltered():
    zu = mel.eval('vrayAddRenderElement zdepthChannel')
    #rename zDepth element
    cmds.setAttr('vrayRE_Z_depth.vrayClassType', 'zDepthUnfiltered', type = 'string')
    zDepthUnfiltered = cmds.rename(zu , 'zDepthUnfiltered')
    cmds.setAttr(zDepthUnfiltered + '.vray_name_zdepth', zDepthUnfiltered, type = 'string')
    cmds.setAttr(zDepthUnfiltered + '.vray_depthFromCamera_zdepth', False)
    cmds.setAttr(zDepthUnfiltered + '.vray_depthClamp', False)
    cmds.setAttr(zDepthUnfiltered + '.vray_filtering_zdepth', False)

def zDepthFiltered():
    zf = mel.eval('vrayAddRenderElement zdepthChannel')
    #rename zDepth element
    cmds.setAttr('vrayRE_Z_depth.vrayClassType', 'zDepthFiltered', type = 'string')
    zDepthFiltered = cmds.rename(zf , 'zDepthFiltered')
    cmds.setAttr(zDepthFiltered + '.vray_name_zdepth', zDepthFiltered, type = 'string')
    cmds.setAttr(zDepthFiltered + '.vray_depthFromCamera_zdepth', False)
    cmds.setAttr(zDepthFiltered + '.vray_depthClamp', False)
    cmds.setAttr(zDepthFiltered + '.vray_filtering_zdepth', True)
...
def createGUI():
    ...
    cmds.checkBox('occlusionRE',label= "Occlusion", value=False)
    cmds.checkBox('fresnelRE', label= "Fresnel", value=False)
    cmds.checkBox('uvRE',label= "UV", value=False)
    cmds.checkBox('rimLightRE',label= "Rim Light", value=False)
    cmds.checkBox('worldPPPRE',label= "World PPP", value=False)
    cmds.checkBox('topDownRE',label= "Top Down", value=False)
    cmds.checkBox('noiseRE',label= "Noise", value=False)
    cmds.checkBox('wireframeRE',label= "Wireframe", value=False)
    cmds.checkBox('curvatureRE',label= "Curvature", value=False)
    cmds.checkBox('zDepthUnfilteredRE',label= "zDepth Unfiltered", value=False)
    cmds.checkBox('zDepthFilteredRE',label= "zDepth Filtered", value=False)
...
def queryValues():
    ...
    zDepthUnfilteredValue= cmds.checkBox('zDepthUnfilteredRE', query = True, value = True)
    zDepthFilteredValue= cmds.checkBox('zDepthFilteredRE', query = True, value = True)
    ...
    if zDepthUnfilteredValue == True:
        zDepthUnfiltered()

    if zDepthFilteredValue == True:
        zDepthFiltered()
#Run the Script
createGUI()
